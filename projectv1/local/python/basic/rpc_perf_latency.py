...
if csv_file_exists_bool:
    ...
    variables = "timestamp,rpc_rtt(us),rpc_ff_rtt(us)" + ",upcxx_version"
...
p.line([datetime.fromtimestamp(timestamp) for timestamp in data['timestamp']], data['rpc_rtt(us)'],legend_label='rpc_rtt',color=next(colors))
p.line([datetime.fromtimestamp(timestamp) for timestamp in data['timestamp']], data['rpc_ff_rtt(us)'],legend_label='rpc_ff_rtt',color=next(colors))
p.xaxis.axis_label = 'Date'
p.yaxis.axis_label = 'Duration (us)'
